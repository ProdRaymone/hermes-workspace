...
export function KnowledgeBrowserScreen() {
  ...
  useEffect(() => {
    ...
    fetch(buildInstanceApiPath('/api/knowledge/config', activeInstanceId))
      .then((r) => r.json())
      ...
      .catch(() => {})
  }, [activeInstanceId, settingsOpen])

  const deferredSearch = useDeferredValue(searchInput)
  const searchTerm = deferredSearch.trim()

  const listQuery = useQuery({
    queryKey: ['knowledge', 'list', activeInstanceId],
    queryFn: () =>
      readJson<KnowledgeListResponse>(
        buildInstanceApiPath('/api/knowledge/list', activeInstanceId),
      ),
  })
  ...
  const readQuery = useQuery({
    queryKey: ['knowledge', 'read', activeInstanceId, selectedPath],
    queryFn: () =>
  ...
  const searchQuery = useQuery({
    queryKey: ['knowledge', 'search', activeInstanceId, searchTerm],
    queryFn: () =>
  ...
  const graphQuery = useQuery({
    queryKey: ['knowledge', 'graph', activeInstanceId],
    queryFn: () =>
      readJson<KnowledgeGraphResponse>(
        buildInstanceApiPath('/api/knowledge/graph', activeInstanceId),
      ),
    enabled: graphOpen,
  })
      ...
      <div className="mx-auto flex w-full max-w-[1200px] min-h-0 flex-col px-4 py-6 sm:px-6 lg:px-8">
        ...
        >
          ...
          <HermesInstanceScopeBanner
            ...
            detail="Knowledge reads the selected Hermes profile. Hermes1/default keeps the legacy local root; Hermes2 and Hermes3 use their WSL profile roots."
          />
        </div>
