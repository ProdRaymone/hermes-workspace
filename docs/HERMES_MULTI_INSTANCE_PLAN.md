...
- Global search session results follow the active instance; Skills search and the direct Skills page now target the selected instance when the selected gateway supports skills.
...
- Knowledge browser APIs and UI are V2 instance-scoped: default/Hermes1 keeps legacy local `~/.hermes` knowledge semantics, while Hermes2/Hermes3 use selected WSL profile config, knowledge root, and GitHub cache paths.
- Skills APIs and UI are V2 instance-scoped for the gateway-backed slice: `/api/skills*`, direct Skills, Dashboard skills widget, global search skills, and Inspector skills now pass the active instance and target the selected Hermes gateway instead of silently reading Hermes1/default.
- Hermes config APIs and the main provider/settings surfaces are V2 instance-scoped: `/api/hermes-config` resolves `?instance=`, default/Hermes1 keeps legacy local `~/.hermes/config.yaml` and `.env`, while Hermes2/Hermes3 read/write selected WSL profile `config.yaml` and `.env` through the redacting profile file adapter.
...
| Knowledge/Memory isolation | Knowledge browser/config                                                        | V2 first slice done            | `/api/knowledge/*` and the Knowledge screen now follow the selected instance. Default keeps legacy local `~/.hermes` knowledge paths; Hermes2/3 use selected WSL profile files and cache roots.                                                                 |
...
| Knowledge/Memory isolation | `/api/skills*`                                                                  | V2 first slice done            | Gateway-backed skills routes now resolve the selected instance and direct Skills, Dashboard, global search, and Inspector pass `activeInstanceId`.                                                                                                              |
| Knowledge/Memory isolation | `/api/hermes-config`                                                            | V2 first slice done            | GET/PATCH resolve the selected instance. Default keeps legacy local `~/.hermes/config.yaml` and `.env`; Hermes2/3 use selected WSL profile files through the redacting adapter. Provider/settings UI calls now pass `activeInstanceId`.                         |
| Knowledge/Memory isolation | MCP/config helpers                                                              | V2 first slice done            | `/api/mcp/servers` now resolves the selected instance, prefers the selected gateway config, and falls back to selected WSL profile `config.yaml` for stopped non-default instances. `/api/mcp/reload`, `/api/config-get`, and `/api/config-patch` are selected-instance scoped and do not fall back to Hermes1.                    |
| Visual-only status         | Dashboard                                                                       | Done                           | Dashboard sessions/status follow active instance; model card reads WSL profile metadata; skills widget now reads the active instance when skills are available. Browser smoke confirmed Hermes2 stopped is shown as stopped and does not inherit Hermes1 state. |
